 ...
 * Copyright © 2019 Guido Maria Serra <guido@whitehat.berlin>, <zeph@fsfe.org>
 *
  ...
  for (c = 0; c < output_info->ncrtc; c++) {
    ...
    if (crtc) {
      ...
      if (!found) {
        ...
        found = True;
      } else rotation &= crtc->crtc_info->rotations;
...
output_can_use_rotation (output_t *output, Rotation rotation) {
  ...
  for (c = 0; c < output_info->ncrtc; c++) {
    crtc_t	*crtc = find_crtc_by_xid (output_info->crtcs[c]);
    if (crtc && !crtc_can_use_rotation (crtc, rotation)) return False;
  } // c++
  return True;
}
...
output_is_primary(output_t *output) {
  if (has_1_3) return XRRGetOutputPrimary(dpy, root) == output->output.xid;
  return False;
}
...
set_gamma_info(output_t *output) {
  ...
  if (last_best == 0)
    ...
    if ((last_best + 1) == size) output->brightness = v2;
...
set_output_info (output_t *output, RROutput xid, XRROutputInfo *output_info) {
  ...
  if (!(output->changes & changes_mode)) {
    ...
    if (crtc && crtc->crtc_info)
      ...
      else set_name_xid (&output->mode, None);
    ...
    else output->mode_info = find_mode_for_output (output, &output->mode);
  ...
  if (!(output->changes & changes_transform)) {
    ...
    if (output->scale_from_w > 0 && output->mode_info) {
      ...
      if (sx != 1 || sy != 1) output->transform.filter = "bilinear";
      else output->transform.filter = "nearest";
      
      output->transform.nparams = 0;
      output->transform.params = NULL;
    } // output->scale_from_w > 0 && output->mode_info
  } // [else] !(output->changes & changes_transform

  /* set primary */
  if (!(output->changes & changes_primary)) output->primary = output_is_primary(output);
} // set_output_info
  ...
  for (i = 1; i < argc; i++) {
    ...
    if (!strcmp ("--gamma", argv[i])) {
      ...
      if (sscanf(argv[i], "%f:%f:%f", &output->gamma.red, 
        &output->gamma.green, &output->gamma.blue) != 3) usage ();
    ...
    if (!strcmp ("--brightness", argv[i])) {
      ...
      if (sscanf(argv[i], "%f", &output->brightness) != 1) usage ();
    ...
    if (!strcmp ("--scale", argv[i])) {
      ...
      if (sscanf (argv[i], "%lfx%lf", &sx, &sy) != 2) usage ();
    ...
    if (!strcmp ("--fbmm", argv[i])) {
      ...
      if (sscanf (argv[i], "%dx%d",
        &fb_width_mm, &fb_height_mm) != 2) usage ();
    ...
    if (!strcmp("--setprovideroutputsource", argv[i])) { 
      ...
      if (++i>=argc) output_source_provider_xid = 0;
      else output_source_provider_xid = check_strtol(argv[i]);

      if (provider_xid == 0) usage();
    ...
    if (!strcmp("--setprovideroffloadsink", argv[i])) { 
      ...
      if (++i>=argc) offload_sink_provider_xid = 0;
      else offload_sink_provider_xid = check_strtol(argv[i]);
